...
class DataLoader:
    ...
    def load(self, input_directory=None):
        ...
        file_list = sorted(Path(input_dir).glob("*.csv"))[: self.max_files]

        @ray.remote
        def parse_file_remote(file_path_str, skip_rows, light_col):
            ...
            dataframe = pd.read_csv(
                file_path,
                skiprows=skip_rows,
                names=["bin", "x", "y", "z", light_col],
                comment="b",
                dtype={"x": float, "y": float, "z": float, light_col: float},
            )
            return dataframe, coordinates
        
        self.logger.section("[Data Loading]")
        self.logger.subsection(f"Loading {len(file_list)} files in parallel with Ray")
        futures = [parse_file_remote.remote(str(fp), self.header_rows_to_skip, self.light_column) for fp in file_list]
...
class DataProcessor:
    ...
    def detect_spatial_outliers(self, dataframes, zscore_threshold, radius):
        ...
        def detect_outliers_single(df, zscore_threshold, radius):
            ...
            for sensor_index, neighbor_indices in enumerate(all_neighbors):
                ...
                if neighbor_indices:
                    ...
                    mean            = neighbor_values.mean()
                    std             = neighbor_values.std()
        ...
        chunk_size   = 100  
        self.logger.subsection(f"Processing {len(dataframes)} dataframes in chunks of {chunk_size}")
    ...
    def scale_light_intensity(self, dataframes, scale_factor):
        ...
        scaled = []
        batch_size = min(50, max(10, len(futures) // 20))
        self.logger.subsection(f"Scaling {len(dataframes)} dataframes using batch size {batch_size}")
